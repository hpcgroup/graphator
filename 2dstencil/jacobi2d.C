...
int main(int argc, char **argv) {
  ...
  double data;
  for(myRank = 0; myRank < num_blocks_x*num_blocks_y; myRank++) {
    int myRow = myRank / num_blocks_y;
    int myCol = myRank % num_blocks_y;
#if DO_COMM
    /* Receive my right, left, bottom and top edge */
    MPI_Irecv(&data, 1, MPI_DOUBLE, calc_pe(myRow, wrap_y(myCol+1)), RIGHT, MPI_COMM_WORLD, &req[RIGHT-1]);
    MPI_Irecv(&data, 1, MPI_DOUBLE, calc_pe(myRow, wrap_y(myCol-1)), LEFT, MPI_COMM_WORLD, &req[LEFT-1]);
    MPI_Irecv(&data, 1, MPI_DOUBLE, calc_pe(wrap_x(myRow+1), myCol), BOTTOM, MPI_COMM_WORLD, &req[BOTTOM-1]);
    MPI_Irecv(&data, 1, MPI_DOUBLE, calc_pe(wrap_x(myRow-1), myCol), TOP, MPI_COMM_WORLD, &req[TOP-1]);


    /* Send my left, right, top and bottom edge */
    MPI_Send(&data, 1, MPI_DOUBLE, calc_pe(myRow, wrap_y(myCol-1)), RIGHT, MPI_COMM_WORLD);
    MPI_Send(&data, 1, MPI_DOUBLE, calc_pe(myRow, wrap_y(myCol+1)), LEFT, MPI_COMM_WORLD);
    MPI_Send(&data, 1, MPI_DOUBLE, calc_pe(wrap_x(myRow-1), myCol), BOTTOM, MPI_COMM_WORLD);
    MPI_Send(&data, 1, MPI_DOUBLE, calc_pe(wrap_x(myRow+1), myCol), TOP, MPI_COMM_WORLD);
